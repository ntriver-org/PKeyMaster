<#
.SYNOPSIS
    Performs online product key certification (PKC) against the Microsoft SL certification service.

.DESCRIPTION
    Builds a SOAP request to Microsoft's SLCertifyProduct endpoint to check whether
    a product key is valid and not blocked. Certification doesn't consume an activation slot.
...
.PARAMETER PassThru
    Returns a structured PSObject with the certification result and raw SOAP payloads.
...
.EXAMPLE
    .\KeyCertification.ps1 -ProductKey "XXXXX-XXXXX-XXXXX-XXXXX-XXXXX" -ActConfigId "msft2009:..."
#>
[CmdletBinding()]
param(
...
function Submit-KeyCertificationRequest([string]$RequestFull) {
    # Send SOAP and parse the response.
    $out = New-ResponseObject
    $out.RequestFull = $RequestFull

    $url = "https://activation.sls.microsoft.com/slpkc/SLCertifyProduct.asmx"
    $action = "http://microsoft.com/SL/ProductCertificationService/IssueToken"
...
function Invoke-KeyCertificationRequest($ProductKey, $ActConfigId) {
    # Build and send the certification SOAP.
...
"@
    ...
    return Submit-KeyCertificationRequest $requestFull
}
